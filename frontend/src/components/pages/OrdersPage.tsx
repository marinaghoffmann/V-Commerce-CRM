...
const columns = [
  ...
  { key: "categoria_produto", label: "Categoria" },
  {
    ...
    render: (value: string) => (
      <span className="capitalize">
        {value?.toLowerCase() === "cartao" ? "cartão" : value}
      </span>
    ),
  },
  {
  ...
  { key: "quantidade", label: "Qtd." },
  {
    ...
    render: (value: string) => (
      <span
        className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${getStatusStyle(value)}`}
      >
        {value}
      </span>
    ),
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-400">
              Acompanhe todos os pedidos e seus status
            </p>
          </div>
          <button
