...
function ClientDetail(): React.ReactElement {
  ...
  const [isPressingContact, setIsPressingContact] = useState(false);
  ...
  const showCopyFeedback = (type: "success" | "error", message: string) => {
  ...
  const handleCopyContact = async () => {
    setIsPressingContact(true);
    if (pressTimerRef.current) {
      window.clearTimeout(pressTimerRef.current);
    }
    pressTimerRef.current = window.setTimeout(() => {
      setIsPressingContact(false);
    }, 150);

    const telefone = data?.telefone_formatado?.trim();
    if (!telefone) {
      showCopyFeedback("error", "Número indisponível para cópia.");
      return;
    }
    try {
      await copyToClipboard(telefone);
      showCopyFeedback("success", `Número ${telefone} copiado`);
    } catch {
      showCopyFeedback("error", "Não foi possível copiar o número.");
    }
  };
